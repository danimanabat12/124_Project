...
bool errorChecking(vector<string> user_command){																		//bool errorChecking() function created by: Ferdinand A. Gador II
	...
	if(user_command.size()==1){																							//if the size of the vector is 1, 
		...
		if(user_command.at(0)=="("&&(equalCount!=0&&operatorCount!=0)) stringChecker=false;
...
bool processElements(vector<string> user_command, vector<variables> &var_list) {				
	...
	//Check if the first character on the first element on the input vector is an n oru an equal sign. If yes, it will result to an error
	...
	for (i = 0; i < stripedInput.size(); i++) {							
		...
		if (isValid(input) && i > 0 && isValid(stripedInput.at(i-1)) && !isKeyword(stripedInput.at(i-1))) {
			cout << "SNOL> Unknown command! Does not match any valid command of the language." << endl;
			return false; 
		}
	...
	try{																				//block of code to try
		...
		if (isFloat(stringAns) && floatOperands == false) {
			...
			for (int i = 0; i < stringAns.size(); i++) {
				...
				else if (stringAns.at(i) == '-') {
					if (stringAns.at(i+1) != '0') {
						bin += stringAns.at(i);
					}
				}
				else bin += stringAns.at(i);
			}
			stringAns = bin;
		}
